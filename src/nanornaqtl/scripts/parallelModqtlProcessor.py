...
"""
多线程并行处理QTL识别脚本
用于并行执行callmodQTL.py脚本处理所有染色体和链的组合
"""
...
def setup_argparse():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="修饰位点结果CSV文件路径 (例如：*_mod_sites_result.csv)",
    )
    parser.add_argument(
        "-pkl",
        "--read_mod_dict",
        type=str,
        required=True,
        help="修饰read字典PKL文件路径 (例如：*_mod_read_final.pkl)",
    )
    parser.add_argument(
        "--geno_size",
        type=str,
    ...
    parser.add_argument(
        "-c", "--min_coverage", type=int, default=8, help="最小总覆盖度 (默认: 8)"
    )
    parser.add_argument(
...
def run_single_task(task_info):
    ...
    try:
        ...
        callmodqtl_script = os.path.join(current_dir, "callmodQTL.py")
        ...
        if result.returncode == 0:
            # 构建预期的输出文件路径
            output_file = f"{args.output_prefix}_{args.modification_type}QTL_{chrom}_{strand}.csv"
...
def merge_results(output_prefix, modification_type, successful_files, keep_tmp=False):
    """
    合并所有成功的结果文件

    Args:
        output_prefix: 输出文件前缀
        modification_type: 修饰类型
    ...
    try:
        ...
        merged_df = merged_df.sort_values(
            by=["chrom_num", "snp_pos_1base", "mod_pos_1base"],
        ...
        merged_df = merged_df.sort_values(
            by=["chrom", "snp_pos_1base", "mod_pos_1base"], ascending=[True, True, True]
        )

    # 生成最终文件名
    final_output = f"{output_prefix}_{modification_type}_QTLs_result.csv"
