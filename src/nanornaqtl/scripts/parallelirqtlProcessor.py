#!/usr/bin/env python3
"""
多线程并行处理内含子滞留率QTL识别脚本
并行执行callirQTL.py处理所有染色体和链的组合（共44个任务）
"""
...
def setup_argparse():
    ...
    parser.add_argument(
        "-csv", "--ir_csv", type=str, required=True,
        help="*_intronRetention_result.csv路径"
    )
    parser.add_argument(
...
def generate_tasks(args):
    """
    生成所有内含子滞留率QTL分析任务（22条染色体 × 2条链 = 44个任务）
...
def run_single_task(task_info):
    ...
    try:
        ...
        callirqtl_script = os.path.join(current_dir, "callirQTL.py")

        # 构建命令
        cmd = [
            "python", callirqtl_script,
            "-b", bam_file,
        ...
        if result.returncode == 0:
            # 构建预期输出文件路径
            output_file = f"{args.output_prefix}_intronretention_haplotype_{chrom}_{strand}_tmp.csv"
...
def merge_results(successful_files, output_prefix, keep_tmp):
    ...
    # 生成最终文件名并保存
    final_output = f"{output_prefix}_intronRetention_QTLs_result.csv"
...
def main():
    ...
    # 验证输入文件
    required_files = [args.snp_info, args.ir_csv, args.geno_size]
    ...
    current_dir = os.path.dirname(os.path.abspath(__file__))
    callirqtl_script = os.path.join(current_dir, "callirQTL.py")
    if not os.path.exists(callirqtl_script):
        print(f"错误: 脚本文件不存在: {callirqtl_script}")
        sys.exit(1)
